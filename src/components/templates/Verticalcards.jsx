      ...
      {data.map((data, index) => (
        <Link to={`/${data.media_type || title}/details/${data.id}`} className="relative w-[35vh] flex flex-col" key={index}>
          <img
            ...
            alt=""
          />
          <h1 className="text-xl font-medium mt-2 tracking-wide text-zinc-300">
            {data.name ||
              data.original_name ||
              data.title ||
              data.original_title}
        ...
        {  data.vote_average && (
          <div className="absolute -right-7 bottom-16 bg-sky-600 w-14 h-14 rounded-full text-lg font-semibold flex items-center justify-center">
